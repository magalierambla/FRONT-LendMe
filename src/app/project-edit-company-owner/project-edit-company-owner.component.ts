...
export class ProjectEditCompanyOwnerComponent implements OnInit {
  ...
  onFormSubmitUpdateProject() {
    ...
    console.log(
      'this.ObjetProject.porteProject.id = ',
      this.ObjetProject._porte_project.id
    );

    this.onChangeDateLimiteCollecte(this.ObjetProject.date_limite_collecte);

    if (
      this.ObjetProject.contrePartieProject &&
      !this.checkContrePartie &&
      this.isValidDateCollecte
    ) {
      this.apiService
        .updateDataProjectByUser(
          this.ObjetResponseConnection,
          this.ObjetProject
        )
        .subscribe(
  ...
  getinfosProject(tokenProject) {
    ...
    // tslint:disable-next-line:max-line-length
    this.apiService
      .getDataProjectForUser(
        this.ObjetResponseConnection,
        tokenProject,
        'porteur_project'
      )
      .subscribe(
        (dataPorject: ProjectModel) => {
  ...
  deleteMediaByProject(indexObjectAdresse, objectAdresse) {
    // tslint:disable-next-line:max-line-length
    this.apiService
      .deleteAdressReseauSocialProject(
        this.ObjetResponseConnection,
        this.ObjetProject.token,
        objectAdresse
      )
  ...
  getAllImageProject() {
    // tslint:disable-next-line:max-line-length
    this.apiService
      .getAllImagesByIdProject(
        this.ObjetResponseConnection,
        this.ObjetProject.token
      )
      .subscribe(
        (dataImages: Array<ImageProjectModel>) => {
