...
import {ImageProjectModel, StatutProjectModel, CategorieProjectModel, AdressReseauxSociauxProjectModel, UserModel,
   ProjectModel, ResponseConnectionUserModel, ProjectRequestModel, PorteProjectModel, ErrorMessageValidation   } from '../interfaces/models';
...
@ViewChild('recaptcha', {static: true }) recaptchaElement: ElementRef;
     ...
     public listMessagesErreur: Array<ErrorMessageValidation> = [];

     public ObjetProjectReq: ProjectRequestModel = new ProjectRequestModel();

     public ObjetProjectResp: ProjectModel = new ProjectModel();
  ...
  addEventDateLimiteCollecte(event){
    ...
    if (new Date(event) < date){
      ...
      this.ObjetProjectReq.date_limite_collecte = this.datePipe.transform(event, 'yyyy-MM-dd');

      console.log(this.ObjetProjectReq.date_limite_collecte);
  ...
  handleChange(value){
    ...
    this.ObjetProjectReq.contrePartieProject = value;
  ...
  onChangeCategorieProject(index){
    ...
    this.ObjetProjectReq.token_category = this.listCategorieProject[index].token;
  }
  ...
  onChangePorteProject(index){
    ...
    this.ObjetProjectReq.id_porte_project = this.listPorteProject[index].id;
  }
  ...
  onFormSubmitAddProject(){
    ...
    console.log('this.ObjetProject.contrePartieProject = ', this.ObjetProjectReq.contrePartieProject);

    if (this.ObjetProjectReq.contrePartieProject === undefined){
    ...
    if (this.ObjetProjectReq.contrePartieProject  && this.isvalidCaptcha    &&  !this.checkContrePartie){

      // tslint:disable-next-line:max-line-length
      this.apiService.addProjectByCompanyOwner(this.ObjetResponseConnection, this.ObjetProjectReq).subscribe((dataProject: ProjectModel) => {

        console.log(dataProject);

        this.ObjetProjectResp = dataProject;
        ...
        if (this.photosProject.length > 0){
             ...
             for (let index = 0; index < this.photosProject.length; index++) {
                 ...
                 this.addImageProject( this.ObjetProjectResp.token, this.photosProject[index]);
             }
        }
        ...
        if (this.adressReseauxSociauxProject.length > 0){
          ...
          for (let index = 0; index < this.adressReseauxSociauxProject.length; index++) {
              ...
              this.addAdressReseauSocialProject( this.ObjetProjectResp.token, this.adressReseauxSociauxProject[index]);
          }
       }
        ...
        this.router.navigate(['/user-my-projetcs']);
          ...
          if (ObjectError.isArray()){

            this.listMessagesErreur = ObjectError;
            // alert(("ObjectError est un tableau"));

          }else{
            // alert(("ObjectError n'est pas un tableau"));
          }
...
deleteMediaByProject(indexObjectAdresse , objectAdresse){
  ...
  addImageAfficheProject(){
    ...
    if (this.imageFile){
      ...
      this.ObjetProjectReq.afficheProject = imageData.data.link;
  ...
  getListCategorieProject(){
    ...
    this.apiService.getListCategorieProject().subscribe((data: Array<CategorieProjectModel>) => {
  ...
  getListPorteProject(){
    ...
    this.apiService.getListPorteProject().subscribe((data: Array<PorteProjectModel>) => {
