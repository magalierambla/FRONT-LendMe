...
import {UserModel, InscriptionUserModel, ConnectionUserModel, ResponseConnectionUserModel, ErrorMessageValidation} from '../interfaces/models';
  ...
  public onFormSubmitLogin() {
   ...
   /* this.cookie.set('infosConnectionUser', JSON.stringify(this.ObjetLogin));
    ...
    this.apiService.identificationUser(this.ObjetLogin).subscribe((dataUser: ResponseConnectionUserModel) => {
      ...
      this.tinyAlert('L\'identification a échoué.Veuillez réessayer svp.');
    });
  ...
  public onFormSubmitInscription() {
    ...
    this.apiService.inscriptionUser(this.ObjetInscription).subscribe((dataUser: UserModel) => {
        ...
        this.tinyAlert('Un email de confirmation à été envoyé dans votre boite email pour valider votre inscription.');
            ...
            if (ObjectError.message.indexOf('date_naissance')){

                 this.tinyAlert('Le format de la date naissance est incorrecte');

            }else if (ObjectError.message.indexOf('sex')){

                 this.tinyAlert('Le type de votre sex est incorrecte');

            }else{

              this.tinyAlert(ObjectError.error.message);
            }
