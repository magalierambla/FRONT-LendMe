...
export class IdentificationComponent implements OnInit {
  ...
  addEventDateNaissance(event) {
    ...
    if (event >= date) {
      ...
      this.tinyAlert(
        'La date de naissance doit etre inferiur à la date actuelle !!!'
      );
    } else {
      ...
      this.ObjetInscription.date_naissance = this.datePipe.transform(
        event,
        'yyyy-MM-dd'
      );
    }
  ...
  public getDataUserCurrent(
    ...
    this.apiService.getDataUserCurrent(ObjetResponseConnection).subscribe(
      ...
      (error: any) => {
        this.tinyAlert(
          "Une erreur produite dans l'enregistrement .Merci de relancer l'inscription."
        );
      }
    );
  }
  ...
  public onFormSubmitInscription() {
    if (!this.isErreurInscription) {
      ...
      this.apiService.inscriptionUser(this.ObjetInscription).subscribe(
        (dataUser: UserModel) => {
          ...
          if (!dataUser) {
            ...
            this.tinyAlert(
              'Un email de confirmation à été envoyé dans votre boite email pour valider votre inscription.'
            );
          }
        },
        (error: any) => {
